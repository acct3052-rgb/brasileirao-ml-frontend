...
function opportunityScore(f: Fixture) {
  return (f.confidence ?? 0) * 0.6 + (f.over_15_prob ?? 0) * 0.4
}

function qualifies(f: Fixture) {
  return (f.confidence ?? 0) >= 0.60 || (f.over_15_prob ?? 0) >= 0.70
}

function sortByRoundThenScore(a: Fixture, b: Fixture) {
...
export function FixturesWithLineups({ fixtures }: Props) {
  ...
  const highOpportunity = [...fixtures].filter(qualifies).sort(sortByRoundThenScore)
  const rest = [...fixtures].filter((f) => !qualifies(f)).sort(sortByRoundThenScore)
    ...
    return (
      <div key={f.match_id}>
        ...
        >
          ...
          <span className="text-xs text-muted-foreground whitespace-nowrap">{formatDate(f.match_date)}</span>
        ...
        {isExpanded && (
          <div className={cn(
            ...
            getCalibrationTier(f.confidence).highlight === 'hot' ? 'bg-emerald-500/5' : 'bg-muted/10'
          )}>
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div key={outcome} className={cn(
                  ...
                  f.predicted_result === outcome ? 'border-blue-500/30 bg-blue-500/5' : 'border-border bg-card'
                )}>
                  <div className="text-[11px] text-muted-foreground truncate">{label}</div>
                  <div className="text-xl font-bold tabular-nums">{Math.round(prob * 100)}%</div>
  ...
  return (
    <div className="space-y-6">
      ...
      {highOpportunity.length > 0 && (
        <div>
          <p className="text-xs font-medium text-emerald-400 mb-2">
            ⭐ Melhores oportunidades — confiança ≥60% ou Over 1.5 ≥70%
          </p>
          <div className="rounded-lg border border-emerald-500/20 overflow-hidden divide-y divide-border">
            {highOpportunity.map(renderFixture)}
          </div>
        </div>
      )}
      ...
      {rest.length > 0 && (
        <div>
          <p className="text-xs text-muted-foreground mb-2">Demais jogos</p>
          <div className="rounded-lg border border-border overflow-hidden divide-y divide-border">
            {rest.map(renderFixture)}
          </div>
        </div>
      )}
    </div>
